 ...
 *
 * <p>A {@code RestClientCustomizer} only reaches builders that came from Boot's auto-configured
 * {@code RestClient.Builder} bean. A client built with the static {@code RestClient.builder()}
 * never passes through one, and an application doing that got a puretx that reported nothing
 * and never said why.
 *
 * <p>{@code mutate()} carries over the request factory, the base URL and any interceptors already
 * configured, and the bean stays a {@code RestClient}, so nothing downstream notices. A client
 * that already carries the interceptor — one built from the customized builder bean — is
 * returned untouched rather than reported twice.
 *
...
public final class PuretxRestClientPostProcessor implements BeanPostProcessor {
    ...
    public Object postProcessAfterInitialization(final Object bean, final String beanName) throws BeansException {
        ...
        final RestClient.Builder builder = restClient.mutate();
        if (!interceptor.installOn(builder)) {
            return bean;
        }
